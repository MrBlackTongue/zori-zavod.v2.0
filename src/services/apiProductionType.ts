...
export function getProductionTypeById(
  id: number,
): Promise<TypeProductionType | undefined> {
  return api
    .get(`${PRODUCTION_TYPE}/${id}`)
    .then(response => response.data)
    .catch(handleErrorResponseMessage);
}

// Добавить новый тип производства
export function createProductionType(
  data: TypeProductionType,
): Promise<TypeApiResponse> {
  return api
    .post(PRODUCTION_TYPE, data)
...
export function deleteProductionTypeById(id: number): Promise<TypeApiResponse> {
  return api
    .delete(`${PRODUCTION_TYPE}/${id}`)
    .then(handleResponseDeleteMessage)
    .catch(handleErrorResponseMessage);
}

// Редактировать тип производства
export function updateProductionType(
  data: TypeProductionType,
): Promise<TypeApiResponse> {
  return api
    .put(PRODUCTION_TYPE, data)
